######################################################################################################
#  R-code to replicate the simulation study outlined in Section 3 of the paper (Scenario I: PH and linear)
#
...
#########################################################################################################
# Simulation Scenario I (PH and linear): Only one covariate value and one time point
##########################################################################################################
	#####################################################
	# Input parameters
	#####################################################
	...
	# n = 300
	#####################################################
		m1.300.x0 <- cTimeROC_simulation(n.sim = n.sim, 
			n = 300, 
			cov.value = 1, 
			predict.time = 0.05,
			scenario = "m1", 
		...
		m1.300.x2 	<- cTimeROC_simulation(n.sim = n.sim, 
			n = 300, 
			cov.value = 2, 
			predict.time = 0.05,
			scenario = "m1", 
			...
			fitted.models.ps = m1.300.x0$fitted.models.ps, # Fitted models (hazard and biomarker) for the proposed approach
			lbd = m1.300.x0$lbd, 			# Bandwidths for the non-parametric approach				
			data.sim = m1.300.x0$data.sim) 	# Data used in the original simulation
