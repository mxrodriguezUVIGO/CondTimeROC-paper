######################################################################################################
#  R-code to replicate the simulation study outlined in Section 3 of the paper (Scenario II: NPH and nonlinear)
#
...
##########################################################################################################
# Simulation Scenario II (PH and nonlinear): Only one covariate value and one time point
##########################################################################################################
	...
	# n = 300
	#####################################################
		m2.300.x0 	<- cTimeROC_simulation(n.sim = n.sim, 
			n = 300, 
			cov.value = 0, 
			predict.time = 0.12,
			scenario = "m2", 
		...
		m2.300.x2 	<- cTimeROC_simulation(n.sim = n.sim, 
			n = 300, 
			cov.value = 2, 
			predict.time = 0.12,
			scenario = "m2", 
			...
			fitted.models.ps = m2.300.x0$fitted.models.ps, # Fitted models (hazard and biomarker) for the proposed approach
			lbd = m2.300.x0$lbd, 			# Bandwidths for the non-parametric approach				
			data.sim = m2.300.x0$data.sim) 	# Data used in the original simulation


	#####################################################
	# n = 600
	#####################################################
		m2.600.x0 <- cTimeROC_simulation(n.sim = n.sim, 
			n = 600, 
			cov.value = 0, 
			predict.time = 0.12,
			scenario = "m2", 
